...
public final class KVServer1 extends DefaultRecoverable {
    ...
    private byte[] executeSingle(byte[] command, MessageContext msgCtx, boolean fromConsensus) throws IOException {
        ...
        if (iterations % interval == 0) {
            ...
            float denom = (System.currentTimeMillis() - throughputMeasurementStartTime);
            System.out.println("denom is " + denom);
            float epsilon = 0.000001f;

            if (Math.abs(denom) < epsilon) {
                System.out.println("The float is approximately zero.");
                tp = (float) 0.0001;
            } else {
                System.out.println("The float is not zero.");
            }

            tp = (float) (interval * 1000 / denom);

            System.out.println("--- after calculating tp");
            // tp =
        ...
        if (tuple.type.equals("SEQUENCE")) {
            ...
            this.sequence++;
            System.out.println("--- sequence number: " + this.sequence + "\n");
