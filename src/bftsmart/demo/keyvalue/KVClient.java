...
public class KVClient {
    ...
    public KVClient() {
        // logger = Logger.getLogger(KVClient.class.getName());
    }
    ...
    public static void passArg(String[] args) throws IOException {
        ...
        System.out.println("-------initID " + Integer.parseInt(args[0]) + "\n");
        ...
        System.out.println("------- calling ReplyListener\n");
        Security.addProvider(new BouncyCastleProvider());
        KVProxy = new ReplyListener(initID, "./library/config/");
    ...
    public static void sendRequest(String input) throws IOException {
        try {
            ...
            for (int i = 0; i < 1; i++) {
                ...
                KVProxy.invokeAsynchRequest(assembleRequest("SEQUENCE", Integer.toString(initID), tmp), null,
    ...
    public static byte[] assembleRequest(String type, String channelID, byte[] payload) throws IOException {
        ...
        PrivateKey key = KVProxy.getViewManager().getStaticConf().getPrivateKey();
    ...
    private static class ClientThread extends Thread {
        ...
        public ClientThread(int id) throws IOException {
            // Sisi: For now we just use random bytes. Later it needs to be extended to
            // receive bytes either directly from
            // Python script or through a thread/buffer
            this.id = id;
            this.out = new AsynchServiceProxy(this.id, "./library/config/");
        }
        ...
        private static byte[] readBytes(DataInputStream is) throws IOException {
            ...
            // logger.log(Level.INFO, "Read number of bytes: " + size + "\n");
        ...
        public void run() {
            ...
            try {
                ...
                this.out.invokeAsynchRequest(serializeRequest("CONFIG", id, env),
                        new bftsmart.communication.client.ReplyListener() {
                            ...
                            public void replyReceived(RequestContext rc, TOMMessage tomm) {
                                ...
                                double q = Math.ceil((double) (out.getViewManager().getCurrentViewN()
                                        + out.getViewManager().getCurrentViewF() + 1) / 2.0);
